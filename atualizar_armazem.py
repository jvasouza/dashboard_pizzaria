...
REPO_DIR = Path(r"C:\Users\jvand\Desktop\Dashboard\dashboard_armazem")
...
def git_commit_push(repo_dir, msg):
    ...
    res = subprocess.run(["git", "-C", str(repo_dir), "commit", "-m", msg], capture_output=True, text=True)
...
def main():
    print("== Atualizando Armazém Benevenuto ==")
    itens   = escolher_mais_recente(INBOX_DIR, parse_date_end_itens,   "Historico_Itens_Vendidos")
    pedidos = escolher_mais_recente(INBOX_DIR, parse_date_end_pedidos, "Todos os pedidos")
    receber = escolher_mais_recente(INBOX_DIR, parse_date_end_receber, "Lista-contas-receber")
    ...
    msg = f"Atualização automática ARMAZÉM - {datetime.now():%Y-%m-%d %H:%M}"
